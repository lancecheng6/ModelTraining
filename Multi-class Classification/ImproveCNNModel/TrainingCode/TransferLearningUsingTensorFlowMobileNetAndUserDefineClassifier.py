...
# 7. Load pre-trained model directly, excluding the final classification layer
base_model = applications.MobileNetV2(
    input_shape=(192, 192, 3),
    include_top=False, # Do not include the original 1000-class classifier
    weights='imagenet',
    pooling='avg'      # Equivalent to automatically connecting GlobalAveragePooling2D
)

base_model.trainable = True # Set to trainable (Fine-tuning)

# 8. Append a classifier after MobileNet
hub_model = models.Sequential([
    base_model,
...
tf_board_path = '/kaggle/working/tf_dir/hub_model'
shutil.rmtree(tf_board_path, ignore_errors=True)

# 9. Start training
hub_model_his = hub_model.fit(
...
# 10. Graphically display the accuracy during the training process
visualize_keras_history(hub_model_his)


# Create directory
model_folder = pathlib.Path('/kaggle/working/flower_recognizer/mobile_net')
model_folder.mkdir(parents=True, exist_ok=True)

# 11. Save model
hub_model.save(os.path.join(model_folder, 'TransferLearningUsingTensorFlowMobileNetAndUserDefineClassifier.h5'))
