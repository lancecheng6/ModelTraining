...
def read_data_as_pd(file_path: str) -> pd.DataFrame:
    """Read dataset into DataFrame format
...
def process_query(query: str) -> List[str]:
    """ Preprocess query, e.g.: ‘I want to watch a movie’ -> ['I', 'want', 'to', 'watch', 'movie']
...
class Processor(object):
    ...
    def build_token_dict(self, corpus: List[List[str]]):
        """
        Build token dictionary. This method iterates through the segmented corpus to build
        a token frequency dictionary and a token-to-index mapping dictionary.
        Args:
    ...
    def convert_text_to_index(self, sentence: List[str]):
        """
        Convert the token array into a corresponding index array
    ...
    def build_from_w2v(self, w2v_path: str):
        ...
        Args:
            w2v_path: Path to the pre-trained word embedding file
        """
        w2v = gensim.models.KeyedVectors.load_word2vec_format(w2v_path)

        token2idx = {
            '<PAD>': 0, # Since we use 0 to pad sequences, the pad index must be 0
            '<UNK>': 1  # The index for unknown words can be anything; 1 is set for convenience
        }

        # Iterate through the pre-trained word embedding vocabulary and add to our index dictionary
        for token in w2v.index_to_key:
            token2idx[token] = len(token2idx)

        # Initialize an all-zero tensor with shape [total tokens, pre-trained vector dimension]
        vector_matrix = np.zeros((len(token2idx), w2v.vector_size))
        # Randomly initialize the tensor for the <UNK> token
    ...
    def save_processor(self, folder: str):
        """
        Save Processor information to the target folder
        Args:
...
processor = Processor()
processor.build_from_w2v('/kaggle/input/datasets/lancecheng666888/ww22vv/sgns.weibo.bigram-char')
processor.build_label_dict(list(train_df.label) + list(test_df.label))

# Convert tokenized query to corresponding index array
...
model = tf.keras.Sequential([
    L.Embedding(input_dim=len(processor.token2idx),
                output_dim=processor.w2v.vector_size,
                weights=[processor.vector_matrix],
                input_shape=(15,),
                trainable=False),
...
# 4. Save model
model.save('/kaggle/working/outputs/PreTrainedEnbedding_model.h5')

# 5. Save Processor
processor.save_processor('/kaggle/working/outputs/processorPreTrainedEnbedding')

# Load Processor
#loaded_processor = Processor()
#loaded_processor.load_processor('/kaggle/working/outputs/processorPreTrainedEnbedding')
#
# Load model
# Define a custom object mapping, telling Keras to use standard softmax when 'softmax_v2' is encountered
#custom_objects = {'softmax_v2': tf.keras.activations.softmax}
# Pass custom_objects during load_model
#loaded_model = tf.keras.models.load_model(
#    '/kaggle/working/outputs/PreTrainedEnbedding_model.h5', 
